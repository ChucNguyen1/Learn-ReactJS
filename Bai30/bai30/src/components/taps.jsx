import React, { useState, useEffect } from "react";

function Tabs() {
  const [data, setData] = useState([]);
  const [tabActive, setTabActive] = useState("products");
  useEffect(() => {
    fetch(`https://dummyjson.com/${tabActive}`)
      .then((res) => res.json())
      .then((data) => {
        if (data.products) setData(data.products);
        else if (data.users) setData(data.users);
        else if (data.posts) setData(data.posts);
      })
      .catch((err) => console.error("Lỗi fetch API:", err));
  }, [tabActive]);

  // Hàm xử lý khi click tab
  const handleChangeTab = (tabName) => {
    setTabActive(tabName);
  };

  return (
    <div className="tabs">
      <div className="tabs__header">
        <div
          className={`tabs__item ${tabActive === "products" ? "active" : ""}`}
          onClick={() => handleChangeTab("products")}
        >
          Products
        </div>
        <div
          className={`tabs__item ${tabActive === "users" ? "active" : ""}`}
          onClick={() => handleChangeTab("users")}
        >
          Users
        </div>
        <div
          className={`tabs__item ${tabActive === "posts" ? "active" : ""}`}
          onClick={() => handleChangeTab("posts")}
        >
          Posts
        </div>
      </div>

      <div className="tabs__content">
        <ul>
          {data.map((item) => (
            <li key={item.id}>
              {item.title
                ? item.title
                : `${item.firstName || ""} ${item.lastName || ""}`}
            </li>
          ))}
        </ul>
      </div>
    </div>
  );
}

export default Tabs;
